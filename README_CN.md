...
![中心线提取示例](images/centerline_demo.png)

**左图 — 完整原始骨架**（勾选"Return Full Raw Skeleton"）：多边形内所有 Voronoi
脊线均被返回。可以清楚看到，在每个钝角弯道处以及细小支流汇入主干处，存在大量短小的
噪声毛刺。

**右图 — 剪枝中心线**（默认模式）：度数感知分支分解算法自动去除末端短分支
（长度 < 参考骨架长度的 10%），同时完整保留所有真实支流交汇点。
左臂弯道、右臂弯道及细小侧支上的虚假短枝均被自动清除。
